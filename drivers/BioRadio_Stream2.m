...
for ch = 1:numEnabledBPChannels
    axis_handles2(ch) = subplot(length(axis_handles2),1,ch);
    if ch==1
        title([char(BioRadio_Name)])
    end
...
xlabel('Frequency (hz)')
linkaxes(axis_handles2,'x')




BioPotentialSignals = cell(1,numEnabledBPChannels);
Y= cell(1,numEnabledBPChannels);
...
%         BioPotentialSignalsFiltered{ch} = [filtered(BioPotentialSignals{ch};myDevice.BioPotentialSignals.Item(ch-1).GetScaledValueArray.double'];
         Lo=length(BioPotentialSignals{ch});
         NFFT = 2^nextpow2(Lo); 
          Y{ch} = fft(BioPotentialSignals{ch},NFFT)/Lo;
         f{ch} = sampleRate_BP/2*linspace(0,1,NFFT/2+1);
         YFFT{ch}= 2*abs(Y{ch}(1:NFFT/2+1));
        cla(axis_handles2(ch))
        plot(axis_handles2(ch),f{ch},YFFT{ch})
